SELECT
	...
	, (SELECT TOP 1 DATEDIFF(DAY, pt.TranDate, GETDATE()) FROM Erp.PartTran pt with(nolock) 
...
FROM
	Erp.PartWhse pw with(nolock)

	INNER JOIN Erp.Warehse whse with(nolock) ON
	...
	INNER JOIN Erp.PartCost pc with(nolock) ON
