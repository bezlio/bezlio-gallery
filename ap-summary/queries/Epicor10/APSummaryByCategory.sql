SELECT
	...
	, ih.DueDate
FROM
	Erp.APInvHed ih with(nolock)

	INNER JOIN Erp.Vendor v with(nolock) ON
	v.Company = ih.Company
	and v.VendorNum = ih.VendorNum

	INNER JOIN Erp.APInvDtl id with(nolock) ON
	...
	LEFT OUTER JOIN Erp.ProdGrup pg with(nolock) ON
